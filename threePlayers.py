...
class Connect4Game3:
    ...
    def run_game(self,myfont,p1name,p2name,p3name):
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEMOTION:
                    ...
                    if self.turn == 0:
                        ...
                        pygame.draw.circle(self.screen, BLUE, (posx, int(self.square_size / 2)), self.rad)
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    ...
                    # Ask player 3 input
                        ...
                        if self.is_valid_location(col):
                            row = self.get_next_open_row(col)
                            self.drop_piece(row, col, 3)

                            if self.winning_move(3):
                                total_wins=0
                                with shelve.open('scores') as shelf:
                                    name_score = shelf.get('name_score', {})
                                    name_score[p3name] += 1
                                    total_wins = name_score[p3name]
                                    shelf['name_score'] = name_score
                                winner = myfont.render(f"{p3name} wins!!", 1, BLUE)
                                winner_total_win = myfont.render(f"Total Wins: {total_wins}", 1, BLUE)
                                self.screen.blit(winner, (self.width//2 - winner.get_width()//2, 5))
                                self.screen.blit(winner_total_win, (self.width//2 - winner_total_win.get_width()//2, 30))
                                self.game_over = True
